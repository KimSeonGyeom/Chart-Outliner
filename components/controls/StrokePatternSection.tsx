...
const StrokePatternSection: React.FC<StrokePatternSectionProps> = ({
  ...
  const handleDashInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const handleStrokePatternChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        {strokeEnabled && (
          <>
            ...
            {strokePattern === 'custom' && (
              <div>
                <label>Custom Dash (format: line,gap)</label>
                <input
                  type="text"
                  value={dashInput}
