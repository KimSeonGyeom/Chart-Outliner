...
import { exportChartVariations } from '../data/exportVariations';
import { downloadChart } from '../controls';

interface ExportVariationsButtonProps {
  chartType: 'bar' | 'line';
  chartRef: React.RefObject<HTMLDivElement>;
  setChartSettings: (settings: any) => void;
}

const ExportVariationsButton: React.FC<ExportVariationsButtonProps> = ({
  chartType,
  chartRef,
  setChartSettings,
}) => {
  ...
  const handleExportClick = async () => {
    ...
    try {
      // Track progress
      const progressTracker = (current: number, totalVariations: number) => {
        setProgress(current);
